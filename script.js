...
function updateRequestsStorage(nextRequests) { saveRequests(nextRequests || requests); }
...
function setPin(name, pin) { const pins = getPins(); pins[normName(name)] = pin; savePins(pins); }
function resetPin(name) { const pins = getPins(); delete pins[normName(name)]; savePins(pins); }
    ...
    setTimeout(() => {
        try {
            ...
            if (editRequestId !== null) {
                ...
                requests.push({ id: Date.now(), user: selectedUser.name.trim().toUpperCase(), team: safeTeam, type: selectedType, start, end, reason, status: "Pending" });
...
function goHome() {
    ...
    // FIX: show login screen again + refresh recent logins
...
function renderRequests() {
    ...
    teams.forEach(teamNum => {
        ...
        teamRequests.forEach(r => {
            ...
            div.innerHTML = `<b>User:</b> ${r.user}<br><b>Type:</b> ${r.type}<br><b>Dates:</b> ${r.start} → ${r.end}<br><b>Reason:</b> ${r.reason||"—"}<br><b>Status:</b> <span style="color:${statusColor};font-weight:bold">${r.status}</span>`;
...
function approveRequest(requestId) {
    ...
    if (r.type === "Leave" && !r.deducted) {
        ...
        if (user) {
            ...
            localStorage.setItem("users", JSON.stringify(users)); renderAdminUserList();
        }
    }
...
function renderMonthView(wrap) {
    ...
    for(let d=1;d<=daysInMonth;d++){
        ...
        const cell=document.createElement("div"); cell.className="cal-cell"+(isToday?" cal-today":"");
...
function renderWeekView(wrap) {
    ...
    days.forEach((d,i)=>{
        ...
        const col=document.createElement("div"); col.className="cal-week-col"+(isToday?" cal-today":"");
...
document.addEventListener("DOMContentLoaded", () => {
    ...
    document.addEventListener("click", (e) => {
        ...
        if (action === "reject") { updateRequestsStorage(latest.map(r => r.id===id ? {...r,status:"Rejected"} : r)); return; }
        ...
        if (action === "purge") { showConfirm("Are you sure? This cannot be undone.", () => updateRequestsStorage(latest.filter(r => r.id!==id))); return; }
        if (action === "editDecision") {
            ...
            if(req.type==="Leave"&&req.status==="Approved"&&req.deducted){
                ...
                if(user){ const days=Math.floor((new Date(req.end)-new Date(req.start))/(1000*60*60*24))+1; user.leave+=days; localStorage.setItem("users",JSON.stringify(users)); renderAdminUserList(); }
                req.deducted=false;
            }
