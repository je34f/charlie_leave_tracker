...
const DEFAULT_USERS = [
    ...
    { name: "CHIANG JIA JIE, DAVIN", leave: 14, team: "HQ"},
    { name: "LUKE SASAYIAH", leave: 14, team: "HQ"},
    { name: "NAH WEI JUN", leave: 14, team: "HQ"},
    ...
    { name: "TEO SHON ZHENG ERNEST", leave: 14, team: "1"},
    { name: "NG YING JIE, RAYNER", leave: 14, team: "1"},
    { name: "REYNOLD CHNG CHOON JUAY", leave: 14, team: "1"},
    ...
    { name: "DEVAN SHAGAR", leave: 14, team: "2"},
    { name: "RAJESH THEJAS", leave: 14, team: "2"},
    ...
    { name: "PHUAN YEAN LOONG, CHESTER", leave: 14, team: "3"},
    { name: "WONG JING YUAN", leave: 1, team: "3"}
...
const defaultTeamByName = new Map(DEFAULT_USERS.map(u => [normName(u.name), String(u.team)]));
...
const PUBLIC_HOLIDAYS = new Set([
    ...
    "2026-05-31","2026-05-27","2026-08-10","2026-11-09","2026-12-25",
]);

const PH_NAMES = {
    "2025-01-01":"New Year's Day","2025-01-29":"CNY Day 1","2025-01-30":"CNY Day 2",
    "2025-04-18":"Good Friday","2025-05-01":"Labour Day","2025-05-12":"Vesak Day",
    "2025-06-07":"Hari Raya Haji","2025-08-09":"National Day","2025-10-20":"Deepavali",
    "2025-12-25":"Christmas Day","2026-01-01":"New Year's Day","2026-02-17":"CNY Day 1",
    "2026-02-18":"CNY Day 2","2026-04-03":"Good Friday","2026-05-01":"Labour Day",
...
function countLeaveDays(start, end) {
    ...
    return { total: days.length, phDays: phDays.length, phNames: phDays.map(d => PH_NAMES[d]), chargeable };
}
...
function resetSessionTimer() {
    ...
    sessionTimer = setTimeout(() => {
        if (selectedUser) {
            showToast("Session expired — please log in again", "#dc3545");
            goHome();
        }
    }, SESSION_TIMEOUT_MS);
}
...
async function updateRequestsStorage(nextRequests) {
    ...
    for (const req of nextRequests) { await sb.upsertRequest(req); }
}
    ...
    try {
        ...
        if (dbRequests) {
            requests = dbRequests.map(r => ({
                ...
                start: r.start ? r.start.slice(0, 10) : r.start,
...
async function saveRequests(next) {
    ...
    for (const req of next) { await sb.upsertRequest(req); }
}
...
function deleteRequestById(id) {
    ...
    localStorage.setItem("requests", JSON.stringify(next));
    requests = next;
    if (isAdmin) renderRequests(); else renderEmployeeRequests();
}
...
function launchConfetti() {
    ...
    function spawnParticle(delayOffset) {
        ...
        Object.assign(el.style, {
            position: "fixed", top: "-16px", left: startX + "px",
            width: size + "px", height: (isCircle ? size : size * 1.6) + "px",
            background: color, borderRadius: isCircle ? "50%" : "3px",
            zIndex: "99999", pointerEvents: "none", opacity: "1",
            transition: "none", transform: "translateX(0px) rotate(0deg)"
        });
    ...
    for (let i = 0; i < 400; i++) {
        spawnParticle(Math.floor(i / 100) * 300 + Math.random() * 400);
    }
}
...
function checkIsAdmin(userName) {
    const isInAdminList = admins.some(a => a.toLowerCase() === userName.toLowerCase());
    const userExists = users.some(u => u.name.toLowerCase() === userName.toLowerCase());
    return isInAdminList && userExists;
}
...
function searchUser() {
    ...
    if (!selectedUser) { selectedUser = users.find(u => u.name.toLowerCase() === input); }

    const info = document.getElementById("userInfo");
    function shakeElement(el) { el.classList.add("shake"); setTimeout(() => el.classList.remove("shake"), 400); }
    ...
    // SECURITY FIX: use checkIsAdmin instead of raw admins.some
    isAdmin = checkIsAdmin(selectedUser.name);

    info.innerHTML = `<p><b>Name:</b> ${sanitize(selectedUser.name)}</p><p><b>Leave Remaining:</b> ${selectedUser.leave}</p>`;
...
function getRecentLogins() { return JSON.parse(localStorage.getItem("recentLogins")) || []; }
...
function renderNotifPanel(notifs) {
    ...
    if (!list) return;
    if (!notifs || notifs.length === 0) { list.innerHTML = `<div class="notif-empty">No notifications</div>`; return; }
    list.innerHTML = "";
    notifs.forEach(n => {
        const item = document.createElement("div");
        ...
        item.innerHTML = `
            <div class="notif-dot ${sanitize(n.type)}"></div>
            <div class="notif-text">${sanitize(n.message)}<div class="notif-time">${timeAgo}</div></div>
...
async function dismissNotif(id) { await sb.deleteNotification(id); await loadNotifications(); }
...
async function createNotification(recipient, message, type) {
    await sb.addNotification({
        id: Date.now() + Math.floor(Math.random() * 1000),
        recipient, message, type, read: false, created_at: Date.now()
    });
}

// ================= PIN SYSTEM (server-side) =================
async function userHasPin(name) { return await sb.userHasPinRemote(name); }
async function verifyPin(name, pin) { return await sb.verifyPinRemote(name, pin); }
async function setPin(name, pin) {
    const ok = await sb.setPinRemote(name, pin);
    if (!ok) throw new Error("Failed to set PIN");
}
async function resetPin(name) { await sb.deletePinRemote(name); }
...
function showPinModal(userName, mode, onSuccess) {
    ...
    overlay.className = "pin-overlay"; overlay.id = "pinModal";
    const box = document.createElement("div"); box.className = "pin-box";
    const title = document.createElement("h3");
    const subtitle = document.createElement("p"); subtitle.className = "pin-subtitle";
    const errorEl = document.createElement("div"); errorEl.className = "pin-error";
    const dotsWrap = document.createElement("div"); dotsWrap.className = "pin-dots";
    const dots = [];
    for (let i = 0; i < 4; i++) {
        const d = document.createElement("div"); d.className = "pin-dot";
    ...
    // SECURITY FIX: handleComplete is now async — verifies PIN server-side
    async function handleComplete() {
        if (phase === "enter") {
            // Show loading state
            errorEl.textContent = "Verifying...";
            const valid = await verifyPin(userName, entered);
            if (valid) { overlay.remove(); onSuccess(); }
            else {
            ...
            if (entered === firstPin) {
                errorEl.textContent = "Saving PIN...";
                try {
                    await setPin(userName, entered);
                    overlay.remove(); onSuccess();
                } catch (err) {
                    errorEl.textContent = "Failed to save PIN. Try again.";
                    firstPin = ""; phase = "create1"; setPhaseUI();
                }
            } else {
...
// SECURITY FIX: initiateLogin is async — checks PIN existence server-side
async function initiateLogin(name) {
    const user = users.find(u => u.name.toLowerCase() === name.toLowerCase().trim());
    if (!user) { showToast("User not found", "#dc3545"); return; }
    showLoadingOverlay(true);
    const hasPin = await userHasPin(user.name);
    showLoadingOverlay(false);
    ...
    showPinModal(user.name, mode, () => {
        ...
        searchUser();
        startSessionTracking(); // ← start 30min session timer
    });
}
        ...
        try {
            ...
            if (editRequestId !== null) {
                ...
                const req = {
                    ...
                    team: safeTeam, type: selectedType,
                    halfDay: isHalfDay ? halfDay : null,
                    start, end, reason, status: "Pending", deducted: false
                };

                if (selectedType === "Leave") {
                    const { chargeable, phDays, phNames } = countLeaveDays(start, end);
                    if (phDays > 0) showToast(`Note: ${phDays} PH day(s) excluded (${phNames.join(", ")}) — ${chargeable} day(s) charged`, "#f59e0b");
                    if (chargeable > 0) {
                        selectedUser.leave = Math.max(0, selectedUser.leave - chargeable);
                        req.deducted = true; req.chargeableDays = chargeable;
                    } else {
                        req.deducted = false; req.chargeableDays = 0;
                    ...
                    document.getElementById("userInfo").innerHTML = `<p><b>Name:</b> ${sanitize(selectedUser.name)}</p><p><b>Leave Remaining:</b> ${selectedUser.leave}</p>`;
                }

                requests.push(req);
                for (const adminName of admins) {
                    await createNotification(adminName, `📋 ${sanitize(selectedUser.name.split(" ")[0])} submitted a ${selectedType} request (${start} → ${end})`, "submitted");
                }
...
function showToast(message, colour = "#28a745") {
    ...
    toast.className = "toast"; toast.style.background = colour; toast.innerText = message;
    document.body.appendChild(toast);
    const existing = document.querySelectorAll(".toast.show");
    let offset = 20;
    existing.forEach(t => { offset += t.offsetHeight + 10; });
    toast.style.top = offset + "px";
    setTimeout(() => { toast.classList.add("show"); toast.style.top = offset + "px"; }, 10);
...
function goHome() {
    ...
    if (isAdmin) {
        if (!document.getElementById("adminSection").classList.contains("hidden") === false) {
            formSection.classList.add("hidden"); adminSection.classList.remove("hidden"); renderRequests(); return;
        }
        formSection.classList.add("hidden"); adminSection.classList.remove("hidden"); renderRequests(); return;
    }

    // CASE 3: employee logout
    stopSessionTracking(); // ← stop session timer on logout
    selectedUser = null; isAdmin = false;
...
async function refreshData() {
    ...
    try {
        const dbUsers = await sb.getUsers();
        if (dbUsers && dbUsers.length > 0) { users = dbUsers; localStorage.setItem("users", JSON.stringify(users)); }
        const dbRequests = await sb.getRequests();
        if (dbRequests) {
            requests = dbRequests.map(r => ({
                ...r, archivedAt: r.archived_at, halfDay: r.half_day || null,
                start: r.start ? r.start.slice(0, 10) : r.start,
                end: r.end ? r.end.slice(0, 10) : r.end
            }));
            localStorage.setItem("requests", JSON.stringify(requests));
...
function renderEmployeeRequests() {
    ...
    filtered.forEach(r => {
        ...
        const halfDayLabel = r.halfDay ? ` <span style="background:#7c3aed;color:#fff;padding:2px 8px;border-radius:20px;font-size:11px;font-weight:700;">${sanitize(r.halfDay)} Off</span>` : "";
        // XSS FIX: all user data wrapped in sanitize()
        div.innerHTML = `
            <div class="request-header"><span class="status ${sanitize(String(r.status).toLowerCase())}">${sanitize(r.status)}</span></div>
            <div class="request-body">
                <b>Type:</b> ${sanitize(r.type)}${halfDayLabel}<br>
                <b>Dates:</b> ${sanitize(r.start)} → ${sanitize(r.end)}<br>
                <b>Reason:</b> ${r.reason ? sanitize(r.reason) : "—"}
            </div>`;
...
function updateUserBanner() {
    ...
    if (oldVal !== "" && oldVal !== String(selectedUser.leave)) {
        countEl.classList.remove("bump"); countEl.offsetHeight; countEl.classList.add("bump");
...
function renderRequests() {
    ...
    const activeTitle = document.createElement("h3"); activeTitle.textContent = "Active Requests"; box.appendChild(activeTitle);
    ...
    teams.forEach(teamNum => {
        ...
        const header = document.createElement("div"); header.className = "teamHeader";
        header.innerHTML = `<h3>Team ${sanitize(teamNum)}</h3><span class="pendingBadge">${pendingCount} Pending</span>`;
        box.appendChild(header);
        if (teamRequests.length === 0) { const noReq = document.createElement("p"); noReq.className = "noReq"; noReq.innerText = "No requests"; box.appendChild(noReq); return; }
        ...
        teamRequests.forEach(r => {
            ...
            const halfDayLabel = r.halfDay ? ` <span style="background:#7c3aed;color:#fff;padding:2px 8px;border-radius:20px;font-size:11px;font-weight:700;">${sanitize(r.halfDay)} Off</span>` : "";
            // XSS FIX: all user data wrapped in sanitize()
            div.innerHTML = `<b>User:</b> ${sanitize(r.user)}<br><b>Type:</b> ${sanitize(r.type)}${halfDayLabel}<br><b>Dates:</b> ${sanitize(r.start)} → ${sanitize(r.end)}<br><b>Reason:</b> ${sanitize(r.reason||"—")}<br><b>Status:</b> <span style="color:${statusColor};font-weight:bold">${sanitize(r.status)}</span>`;
            ...
            btnWrap.appendChild(archiveBtn); div.appendChild(btnWrap); box.appendChild(div);
        });
    });
    ...
    if (archived.length === 0) { const p = document.createElement("p"); p.className="noReq"; p.innerText="No archived requests"; archiveBody.appendChild(p); }
    else {
        archived.sort((a,b) => (b.archivedAt||0)-(a.archivedAt||0));
        archived.forEach(r => {
            const div = document.createElement("div"); div.className = "request";
            div.innerHTML = `<b>User:</b> ${sanitize(r.user)}<br><b>Type:</b> ${sanitize(r.type)}<br><b>Dates:</b> ${sanitize(r.start)} → ${sanitize(r.end)}<br><b>Reason:</b> ${sanitize(r.reason||"—")}<br><b>Status:</b> ${sanitize(r.status)}<br><small>Archived</small>`;
            const btnWrap = document.createElement("div"); btnWrap.className = "btnWrap";
...
async function approveRequest(requestId) {
    ...
    r.status = "Approved";
    await createNotification(r.user, `✅ Your ${r.type} request (${r.start} → ${r.end}) has been approved`, "approved");
    if (r.type === "Leave" && !r.deducted) {
        ...
        if (user) {
            ...
            localStorage.setItem("users", JSON.stringify(users));
            sb.updateUserLeave(user.name, user.leave); renderAdminUserList();
        }
    }
...
function renderAdminUserList() {
    ...
    users.forEach(u => {
        ...
        // XSS FIX: use textContent for name, sanitize for inline html
        row.innerHTML = `
            <span class="userName">
                ${sanitize(u.name)}
            ...
            <div class="leaveEditor">
                <button onclick="changeLeave('${sanitize(u.name)}', -1)">▼</button>
                <span class="leaveCount">${u.leave}</span>
                <button onclick="changeLeave('${sanitize(u.name)}', 1)">▲</button>
                <button onclick="adminResetPin('${sanitize(u.name)}')" style="background:#f59e0b;font-size:11px;padding:4px 8px;">Reset PIN</button>
                <button onclick="removeUser('${sanitize(u.name)}')" style="background:#dc3545;font-size:11px;padding:4px 8px;">Remove</button>
            </div>`;
...
async function addNewUser() {
    ...
    if (users.find(u => normName(u.name) === normName(name))) { showToast("User already exists", "#dc3545"); return; }
    ...
    try {
        ...
        console.error("Failed to sync new user:", err);
        showToast("Added locally but failed to sync", "#f59e0b");
    }
    if (makeAdmin && !admins.includes(name)) admins.push(name);
    nameEl.value = ""; teamEl.value = ""; leaveEl.value = ""; isAdminEl.checked = false;
    renderAdminUserList();
}
...
async function removeUser(name) {
    showConfirm(`Remove ${name.split(" ")[0]} from the system? This cannot be undone.`, async () => {
        ...
        } catch (err) { showToast("Removed locally but failed to sync", "#f59e0b"); }
        renderAdminUserList();
    });
}
...
function changeLeave(name, amount) {
    ...
    localStorage.setItem("users", JSON.stringify(users));
    sb.updateUserLeave(name, user.leave);
    renderAdminUserList();
}
...
function renderWeekView(wrap) {
    ...
    days.forEach((d,i)=>{
        ...
        if(reqs.length===0){ const empty=document.createElement("div"); empty.className="cal-week-empty"; empty.textContent="—"; col.appendChild(empty); }
            ...
            reqs.forEach(r=>{ const pill=document.createElement("div"); pill.className="cal-week-pill"+(r.status==="Pending"?" cal-bar-pending":""); pill.style.background=TEAM_COLORS[r.team]||"#999"; pill.innerHTML=`<span class="cal-pill-name">${sanitize(r.user.split(" ")[0])}</span><span class="cal-pill-type">${sanitize(r.type)}</span>`; col.appendChild(pill); });
        }
...
function initEmployeeCalendar() { renderEmployeeCalendar(); }
function renderEmployeeCalendar() {
    const wrap = document.getElementById("employeeCalendarWrap"); if (!wrap) return;
    if (empCalView === "month") renderEmpMonthView(wrap); else renderEmpWeekView(wrap);
}
...
function renderEmpWeekView(wrap) {
    ...
    const grid=document.createElement("div"); grid.className="cal-week-grid";
    days.forEach((d,i)=>{
        const dateStr=d.toISOString().slice(0,10); const reqs=requestsOnDate(dateStr); const isToday=dateStr===today; const isPH=PUBLIC_HOLIDAYS.has(dateStr);
        const col=document.createElement("div");
        col.className="cal-week-col"+(isToday?" cal-today":"")+(isPH?" cal-ph":"");
        const dayHdr=document.createElement("div"); dayHdr.className="cal-week-day-hdr";
        dayHdr.innerHTML=`<span class="cal-week-dayname">${dayNames[i]}</span><span class="cal-week-datenum">${d.getDate()}</span>`;
...
document.addEventListener("DOMContentLoaded", () => {
    ...
    document.addEventListener("click", async (e) => {
        ...
        if (action === "reject") {
            ...
            if (req && req.type === "Leave" && req.deducted) {
                ...
                if (user) {
                    const refund = req.chargeableDays !== undefined ? req.chargeableDays : countLeaveDays(req.start, req.end).chargeable;
                    user.leave += refund;
            ...
            if (rejReq) await createNotification(rejReq.user, `❌ Your ${rejReq.type} request (${rejReq.start} → ${rejReq.end}) has been rejected`, "rejected");
            return;
        }
    ...
    setInterval(() => { if (selectedUser) loadNotifications(); }, 30000);
    initApp();
});
